...
import { IMAGE_BASE_URL, BACKDROP_SIZE, BACKDROP_SIZE_MEDIUM, POSTER_SIZE } from '../constants';
...
const DetailsPage: React.FC = () => {
  ...
  const handlePlay = () => {
    navigate('/player', { state: { item, type, season: selectedSeason, episode: null } });
  };
  
  const handleEpisodePlay = (episode: Episode) => {
     navigate('/player', { state: { item, type, season: selectedSeason, episode } });
  }
          ...
          <div>
            <div className="relative mb-4">
                <select 
                    ...
                    className="w-full px-4 py-3 text-white bg-[var(--surface)] border-2 border-[var(--border)] rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focusable"
                >
